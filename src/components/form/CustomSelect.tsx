...
const CustomSelect: React.FC<CustomSelectProps> = ({
  ...
  const [selected, setSelected] = useState<OptionSelect | null>(defaultValue);
      ...
      {({ open }) => (
        <>
          ...
          <div className="relative mt-1">
            ...
            >
              <span className="block truncate">{field.value?.name || "Seleziona"}</span>
          ...
          {error && <span className="mt-1 block text-xs text-red-500">This field is required</span>}
        </>
      )}
    </Listbox>
  );
